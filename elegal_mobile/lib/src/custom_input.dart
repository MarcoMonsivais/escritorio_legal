...
class _InputCustomState extends State<InputCustom> {
  ...
  _sendAudio(String path) async {
    ...
    try {
      final reference = FirebaseStorage.instance.ref('e_legal/conf/rooms/' +
          widget.roomIdIn! +
          '/' +
      ...
      final message = types.PartialCustom(
          metadata: {'mimeType': 'audio/m4a', 'name': 'voicenote', 'uri': uri});

      FirebaseChatCore.instance.sendMessage(message, widget.roomIdIn!);
    } catch (onError) {
      print(onError);
    }
  }
                              ...
                              GestureDetector(
                                onTap: () async {
                                  try {
                                    ...
                                    audioSource =
                                        ap.AudioSource.uri(Uri.parse(path2!));
                                    path2 = path2.replaceAll('file:/', '');
                                    _sendAudio(path2);
